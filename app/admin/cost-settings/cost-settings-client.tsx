...
type Draft = { crop: string; box: string; shipping: string; sale: string };
...
export default function CostSettingsClient({ initialSettings, initiallySaved }: { initialSettings: CostSetting[]; initiallySaved: boolean }) {
  ...
  const [drafts, setDrafts] = useState<Record<string, Draft>>(Object.fromEntries(initialSettings.map((item) => [item.product_weight, { crop: String(item.crop_cost), box: String(item.box_cost), shipping: String(item.shipping_cost), sale: String(item.sale_price) }])));
  ...
  function save(weight: string) {
    ...
    startTransition(async () => {
      const response = await updateCostSetting(weight, Number(draft.crop), Number(draft.box), Number(draft.shipping), Number(draft.sale));
  ...
  return <>
    ...
    <section className={styles.grid} aria-label="중량별 비용 설정">
      {settings.map((item) => { const draft = drafts[item.product_weight]; const total = Number(draft.crop || 0) + Number(draft.box || 0) + Number(draft.shipping || 0); return <article key={item.product_weight}><h2>{item.product_weight}</h2><label>판매가 (500원 단위)<input type="number" min="500" max="1000000" step="500" value={draft.sale} onChange={(event) => change(item.product_weight, "sale", event.target.value)} /></label><label>고구마 원가<input type="number" min="0" max="1000000" step="100" value={draft.crop} onChange={(event) => change(item.product_weight, "crop", event.target.value)} /></label><label>박스비<input type="number" min="0" max="1000000" step="100" value={draft.box} onChange={(event) => change(item.product_weight, "box", event.target.value)} /></label><label>배송비<input type="number" min="0" max="1000000" step="100" value={draft.shipping} onChange={(event) => change(item.product_weight, "shipping", event.target.value)} /></label><div><span>예상 잔액</span><strong>{formatPrice(Number(draft.sale || 0) - total)}</strong></div><button type="button" onClick={() => save(item.product_weight)} disabled={isPending}>저장</button></article>; })}
    </section>
  </>;
}
