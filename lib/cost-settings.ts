...
export const fallbackCostSettings: CostSetting[] = [
  { product_weight: "3kg", crop_cost: 6000, box_cost: 500, shipping_cost: 4500, updated_at: null },
  { product_weight: "5kg", crop_cost: 9000, box_cost: 1000, shipping_cost: 5000, updated_at: null },
  { product_weight: "10kg", crop_cost: 16000, box_cost: 1500, shipping_cost: 7000, updated_at: null },
];
...
export async function getCostSettings(): Promise<{ settings: CostSetting[]; saved: boolean }> {
  ...
  const query = new URLSearchParams({ select: "product_weight,crop_cost,box_cost,shipping_cost,updated_at", order: "product_weight.asc" });
  try {
    ...
    if (!response.ok) return { settings: fallbackCostSettings, saved: false };
